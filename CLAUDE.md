...
- `orcamento_notas`, `orcamento_caixa`, `orcamento_setores` - "Orçamento GOCG" module (`/orcamento`, roles `BensPatrimoniais` + `admingeral` only). Notas fiscais (CNPJ + valor required; objeto, setor, militar nome de guerra/RG, observações optional), caixa movements (`tipo`: saque | retorno | ajuste; saldo = saques − retornos ± ajustes − notas **pagas**; a nota carries `pago` + `pagoEm`, missing field = paga, toggled via `PagamentoChip` in the modal and in the table's actions column) and the editable list of destination setores (seeded with DEMOP/SST/SSCO/SSMT/COMANDANTE). Business rules in `src/services/orcamentoService.js`, listeners in `src/hooks/useOrcamento.js`, one file per tab in `src/screens/Orcamento/`. Every typed text is stored in caixa alta. Militar autocomplete comes from `src/data/militaresGocg.js` (N. Guerra + RG only, generated from the DGP PDF by `scripts/gerarMilitaresGocg.py`) plus names already launched.

- `operacoes` - "Materiais das Operações do CBMERJ" (`/operacoes/:id?`, card no Dashboard ao lado do compressor; menu "Operações CBMERJ"). Uma doc por operação/norma: `categoria` (operacao | norma | outra), `icone`, `cor`, `nota`, `boletim`, `folhas`, `vigencia`, `links[]` (Drive), `secoes[]` → `itens[]` (`nome` como consta na nota, `quantidade` fixada pela nota ou `null` = checklist, `unidade`, `observacao`, `material_id` opcional) e `observacoes[]`. Leitura admin/admingeral/BensPatrimoniais; escrita **só admingeral** (rules + UI `podeEditar`). Semente em `src/data/operacoesCbmerj.js` (criada na 1ª abertura pelo admingeral; "Carregar operações que faltam" compara pela `chave`). O vínculo `material_id` é o que traz nome/foto/local em tempo real (`useMaterials` + `useAlocacoesLocais` + `useMateriaisEmViatura`); `avaliarItem()` em `src/services/operacoesService.js` compara `estoque_atual` com a quantidade da nota e gera o aviso sutil "abaixo do previsto". A quantidade prevista nunca é alterada pelo estoque.

### Busca por foto (Claude vision)
`identificarMaterialPorFoto` (Cloud Function, `functions/index.js`) recebe a foto em base64, chama `claude-opus-5` (effort `low`, saída estruturada, `fallbacks: "default"`) e devolve `{ objeto, termos[], categoria, descricao, confianca, modelo, tokens }`. **A lista de materiais nunca vai para a API**: `buscarMateriaisPorTermos()` em `src/services/visaoService.js` procura os termos entre os `materials` já carregados no app (inclui semelhantes). Botão de câmera = `src/components/BuscaPorFoto.jsx` (busca de Material, "Buscar em tudo" do Dashboard e Operações); cada consulta grava `busca_por_foto` no `audit_logs`. Papéis: admin/admingeral/BensPatrimoniais. Requer o segredo `ANTHROPIC_API_KEY` (`firebase functions:secrets:set ANTHROPIC_API_KEY`); **sem ele nenhuma função do codebase faz deploy** (a CLI valida todos os segredos).

### Carga (cheio / vazio) — extintores e cilindros
Separado da inoperância: um extintor vazio está íntegro, só aguarda recarga. Campos no material: `controla_carga` (bool; se ausente é inferido pelo nome em `src/utils/carga.js`) e `qtd_vazios` (unidades do `estoque_atual` que estão vazias; cheios = disponível − vazios). Registro rápido pelo `CargaBadge` na tabela de Material (`CargaDialog`, audit `material_carga`) ou no MaterialDialog; card "vazios/cheios" e filtros "Com vazios (recarga)" / "Com controle de carga" na tela de Material; a cautela avisa (sem bloquear) quando a quantidade passa dos cheios. Vazio NUNCA vira reparo/inoperante automaticamente.
